...
def challenge():
  ...
  if g_request.should_autoaccept():
    if g.team.can_initiate():
      ...
      db.session.add(game)
      set_flash(u"Challenged {}. The game is now in the queue".format(g_request.opponent.name), level='success')
      db.session.commit()
      game.spawn()
    else:
    ...
    set_flash(u"Challenged {}. Since they are lower ELO, they must accept the request.".format(g_request.opponent.name), level='success')
    db.session.commit()
...
def answer_request(request_id):
  ...
  if action == 'reject':
    g_request.reject()
    db.session.commit()
    set_flash(u"Rejected {}'s game request.".format(g_request.challenger.name), level='success')
  elif action == 'accept':
    if g.team.can_initiate():
      ...
      db.session.commit()
      game.spawn()
      set_flash(u"Accepted {}'s game request. It is now in the game queue.".format(g_request.challenger.name), level='success')
    else:
